...
class Container implements ArrayAccess, ContainerInterface {
	...
	public function setProtected(string $id, ?callable $value = null): void {
		if ($value === null) {
			$value = $this->getDefaultDefinition(
				$id,
				sprintf(
					'Attempt to set container ID "%s" as protected, but it is not already set nor provided in the function call.',
					$id
				)
			);
		}
	...
	public function setFactory(string $id, ?callable $value = null): void {
		if ($value === null) {
			$value = $this->getDefaultDefinition(
				$id,
				sprintf(
					'Attempt to set container ID "%s" as factory, but it is not already set nor provided in the function call',
					$id
				)
			);
		}
